...
import { RowTableCustomer } from './data-row-table-customer';
...
export const TableCustomer = () => {
  ...
  const currentPageData = customer.data
    .slice(offset, offset + PER_PAGE)
    .map((data, i) => (
      <RowTableCustomer key={i} number={i + 1} rowData={data} link={`/customers/${data.id}`} />
    ));
  const pageCount = Math.ceil(customer.data.length / PER_PAGE);
