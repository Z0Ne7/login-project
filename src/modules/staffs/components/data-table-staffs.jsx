...
import { RowTableStaffs } from './data-row-table-staffs';
...
export const TableStaffs = () => {
  ...
  useEffect(() => {
    ...
    if (localData) {
      dispatch(action.getDataStaffs(localData));
    }
    setLoading(false);
  }, []);
  const PER_PAGE = 10;
  const offset = currentPage * PER_PAGE;
  const currentPageData = staffs.data
    .slice(offset, offset + PER_PAGE)
    .map((data, i) => (
      <RowTableStaffs key={i} number={i + 1} rowData={data} link={`/staffs/${data.id}`} />
    ));
  const pageCount = Math.ceil(staffs.data.length / PER_PAGE);
