...
import { RowTableProjectStatus } from './data-row-table-project-status';
...
export const TableProjectStatus = () => {
  ...
  const currentPageData = projectStatus.data
    ...
    .map((data, i) => (
      <RowTableProjectStatus
        ...
        number={i + 1}
        rowData={data}
        link={`/project-status/${data.id}`}
      />
    ));
  const pageCount = Math.ceil(projectStatus.data.length / PER_PAGE);
